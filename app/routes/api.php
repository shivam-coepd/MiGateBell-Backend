...
try {
    ...
    if (preg_match('/^\/api\/amenities\/(\d+)$/', $uri, $matches) && $method === 'PUT') {
        (new AmenitiesController)->updateAmenity($matches[1]);
    }

    if (preg_match('/^\/api\/amenities\/(\d+)$/', $uri, $matches) && $method === 'DELETE') {
        (new AmenitiesController)->deleteAmenity($matches[1]);
    }
    ...
    // Accept both PUT and PATCH for status updates (Flutter uses PATCH)
    if (preg_match('/^\/api\/helpdesk\/tickets\/(\d+)\/status$/', $uri, $matches) && in_array($method, ['PUT', 'PATCH'])) {
    ...
    // Update ticket fields (title, description, category, priority) — open tickets only
    if (preg_match('/^\/api\/helpdesk\/tickets\/(\d+)$/', $uri, $matches) && in_array($method, ['PUT', 'PATCH'])) {
        (new HelpdeskController)->updateTicket($matches[1]);
    }

    // Delete ticket — open tickets only, resident owns it or admin
    if (preg_match('/^\/api\/helpdesk\/tickets\/(\d+)$/', $uri, $matches) && $method === 'DELETE') {
        (new HelpdeskController)->deleteTicket($matches[1]);
    }
