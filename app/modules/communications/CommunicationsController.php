...
class CommunicationsController extends BaseController {
  ...
  public function createNotice() {
    try {
      ...
      $errors = $this->validateRequiredFields($data, ['title', 'content', 'priority', 'category']);
      if (!empty($errors)) {
        Response::validationError($errors);
      }
  ...
  /**
   * List notices for the current society (admin can see all, residents see all as well)
   */
  public function getNotices() {
    try {
      $user = $this->auth->authenticate();

      $stmt = $this->db->prepare("SELECT n.*, u.name as authorName FROM notices n LEFT JOIN users u ON n.created_by = u.id WHERE n.society_id = ? ORDER BY n.created_at DESC");
      $stmt->execute([$user['society_id']]);
      $notices = $stmt->fetchAll();

      Response::success("Notices retrieved", $notices);
    } catch (Exception $e) {
      error_log("Get notices error: " . $e->getMessage());
      Response::error("Failed to retrieve notices: " . $e->getMessage(), 500);
    }
  }
  ...
  public function getNotice($id) {
    try {
      $user = $this->auth->authenticate();
      $stmt = $this->db->prepare("SELECT n.*, u.name as authorName FROM notices n LEFT JOIN users u ON n.created_by = u.id WHERE n.id = ? AND n.society_id = ?");
      $stmt->execute([$id, $user['society_id']]);
      $notice = $stmt->fetch();
      if (!$notice) {
        Response::notFound("Notice not found");
      }
      Response::success("Notice retrieved", $notice);
    } catch (Exception $e) {
      error_log("Get notice error: " . $e->getMessage());
      Response::error("Failed to retrieve notice: " . $e->getMessage(), 500);
    }
  }
  ...
  public function updateNotice($id) {
    try {
      ...
      $allowed = ['title', 'content', 'priority', 'category'];
      $updateData = [];
      ...
      if (empty($updateData)) {
        Response::error("No updatable fields provided");
      }

      // Validate lengths if present
      if (isset($updateData['title']) && strlen($updateData['title']) > 200) {
        Response::error("Title must be less than 200 characters");
      }
      if (isset($updateData['content']) && strlen($updateData['content']) > 5000) {
        Response::error("Content must be less than 5000 characters");
      }

      $updated = $this->update('notices', $updateData, 'id = :id', ['id' => $id]);
      if ($updated === 0) {
        Response::error("Failed to update notice", 500);
      }
      Response::success("Notice updated");
    } catch (Exception $e) {
      error_log("Update notice error: " . $e->getMessage());
      Response::error("Failed to update notice: " . $e->getMessage(), 500);
    }
  }
  ...
  public function deleteNotice($id) {
    try {
      $user = $this->auth->authorize('admin');
      $deleted = $this->delete('notices', 'id = ? AND society_id = ?', [$id, $user['society_id']]);
      if ($deleted === 0) {
        Response::error("Failed to delete notice", 500);
      }
      Response::success("Notice deleted");
    } catch (Exception $e) {
  ...
  public function createAnnouncement() {
    try {
        ...
        if (!empty($data['target_group_id'])) {
            $stmt = $this->db->prepare("SELECT id FROM groups WHERE id = ? AND society_id = ?");
