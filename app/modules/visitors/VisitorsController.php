...
class VisitorsController extends BaseController {
  ...
  public function addVisitor() {
    try {
      ...
      if (!empty($data['visit_time'])) {
        if (!preg_match('/^\d{2}:\d{2}:\d{2}$/', $data['visit_time'])) {
  ...
  public function getVisitors() {
    try {
      ...
      $visitorType = isset($_GET['visitor_type']) ? $_GET['visitor_type'] : null;
      ...
      if ($visitorType) {
        $whereClause .= " AND v.visitor_type = :visitor_type";
        $params['visitor_type'] = $visitorType;
      }
  ...
  public function updateVisitorStatus($id) {
    try {
      // Guards, admins, and residents can update visitor status
      $user = $this->auth->authorizeAny(['guard', 'admin', 'resident']);
      ...
      // Residents are restricted to pending, approved, or rejected
      if ($user['role'] === 'resident') {
        $allowedResidentStatuses = ['pending', 'approved', 'rejected'];
        if (!in_array($data['status'], $allowedResidentStatuses)) {
          Response::forbidden("Residents can only set status to: " . implode(', ', $allowedResidentStatuses));
        }
      }
      
      // Check if visitor exists and belongs to the same society
      $stmt = $this->db->prepare("SELECT id, status, resident_id FROM visitors WHERE id = ? AND society_id = ?");
      ...
      // Residents can only update status for their own visitors
      if ($user['role'] === 'resident' && $visitor['resident_id'] != $user['uid']) {
        Response::forbidden("Residents can only update status for their own visitors");
      }
  ...
  public function deleteVisitor($id) {
    try {
      // Residents and Admins can delete visitors
      $user = $this->auth->authorizeAny(['admin', 'resident']);
      
      // Check if visitor exists and belongs to the same society
      $stmt = $this->db->prepare("SELECT id, resident_id FROM visitors WHERE id = ? AND society_id = ?");
      $stmt->execute([$id, $user['society_id']]);
      $visitor = $stmt->fetch();
      ...
      // Residents can only delete their own visitors
      if ($user['role'] === 'resident' && $visitor['resident_id'] != $user['uid']) {
        Response::forbidden("Residents can only delete their own visitors");
      }
